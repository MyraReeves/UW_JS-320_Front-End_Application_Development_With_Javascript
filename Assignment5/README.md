...
* The project must be a React application, bootstrapped with `Vite`
...
* Your app must be your own work. You may work in a group of up to **3 people** if you'd like, but be aware that you'll need to come up with a strategy for working together. Most developers on small projects create branches for new features/fixes, then merge them back into the `main` branch. Merge conflicts can arise, and you'll need to resolve them as you go.
  ...
  * App should do something and be working in a publicly avalible url.
  * App must fetch an exeternal api, whether that is a publicly available API or a database that you have created (for example firebase), must use fetch API, not external fetching libaries.
  * App must use routes, at minimum 2 routes and should use React Router.
  * Your app should be deployed onto a web server and publicly accessible.
  * App should use prop types
  * Incorporates at least one unit test
  * App uses modern JS (Functional components, Arrow functions, etc)
  * Work on feature branches (i.e. `development`) and merge in code into `main`. Pull requests allow us to make comments
